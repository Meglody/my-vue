...
class Compile {
    ...
    // input函数指令
    input(exp) {
        this.$el.addEventListener('onchange', this.$vm[exp])
    }
    ...
    isDirective(attrName) {
        return attrName.indexOf('k-') === 0 && attrName.indexOf('k-on:') < 0
    }

    isEvent(attrName) {
        return attrName.startsWith('@') || attrName.indexOf('k-on:') === 0
    }

    eventName(attrName) {
        return attrName.indexOf('@') >= 0 ? attrName.slice(1) : attrName.slice(5)
    }
}
